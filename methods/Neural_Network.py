...
for team in teams:
    ...
    # encoded_Y = encoder.fit_transform(Y)  # Assuming Y is your target variable and it's categorical
    ...
    #     Dropout(0.1),
    #     Dense(Y_train.shape[1], activation='softmax')  # Adjust the output layer according to the number of classes
    # ])
    ...
    # history = model.fit(X_train, Y_train, epochs=200, batch_size=20,  # Adjusted batch size and epochs
...
plt.figure(figsize=(20, 8))  # Set a larger figure size to accommodate all team names
bars = plt.bar(teams, test_accuracies, color='skyblue')

# Add the exact accuracy above each bar
for bar in bars:
...
# bars = plt.bar(teams, test_accuracies, color='skyblue')  # Create a bar chart

# # Adding the exact accuracy on top of each bar
# for bar in bars:
#     yval = bar.get_height()
...
# plt.title('Neural Network Accuracies By Team (Dropout)')  # Set the title of the plot
# plt.ylim(0.45, 0.76)  # Adjust the limit for the y-axis to improve visibility of text if needed
...
# # Confusion Matrix (For Regular Neural Net)
